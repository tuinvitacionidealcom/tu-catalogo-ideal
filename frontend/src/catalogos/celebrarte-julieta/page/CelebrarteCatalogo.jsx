...
// ImportaciÃƒÂ³n de imÃƒÂ¡genes locales
...
const LOCAL_STORAGE_PRODUCTS_KEY = 'celebrarte_products_v2';
...
const celebrarteCatalogo = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Mostrar scroll-up si el usuario estÃƒÂ¡ scrolleando hacia arriba Y pasÃƒÂ³ los 300px
      ...
      // Detectar si el scroll llegÃƒÂ³ al bottom de la web (menos de 150px del final)
      ...
      if (outOfHero && !wspMessageDismissed) {
        ...
        // Ocultar automÃƒÂ¡ticamente el globo flotante de sugerencia despuÃƒÂ©s de 6 segundos
  ...
  return (
    <div className="min-h-screen bg-web-bg-warm pb-0 text-slate-800 font-sans relative">
      ...
      >
        ...
        {/* Lupa de BÃƒÂºsqueda interactiva que se expande hacia la izquierda y WhatsApp Outline */}
        <div className="flex items-center gap-2 shrink-0">
          {/* Input de BÃƒÂºsqueda Desplegable (Posicionado antes para expandirse hacia la izquierda) */}
          ...
          <a
            href={`https://wa.me/${info.phone}?text=${'%C2%A1Hola%20Julieta!%20Me%20gustar%C3%ADa%20hacer%20una%20consulta%20sobre%20tus%20servicios.'}`}
            target="_blank"
            ...
            <div className="bg-white lg:rounded-3xl shadow-xs overflow-hidden lg:border border-slate-100">
              ...
              <div id="seccion-productos" className="px-6 pt-6 pb-2">
                <div className="relative">
                  <input
                    type="text"
                    placeholder="Buscar servicios, opciones o detalles..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
              ...
              <div className="px-6 py-6">
                {filteredProducts.length === 0 ? (
                  <div className="text-center py-12">
                    <p className="text-slate-400 text-sm">No encontramos servicios que coincidan con tu búsqueda.</p>
                  </div>
                ) : (
      ...
      <div className={`fixed bottom-6 inset-x-4 z-45 max-w-sm mx-auto bg-brand/95 backdrop-blur-md border border-white/10 rounded-full py-2.5 px-4 flex items-center justify-between shadow-2xl shadow-brand/40 lg:hidden gap-2 transition-all duration-300 transform ${
        ...
        {/* Centro: BotÃƒÂ³n de Carrito DinÃƒÂ¡mico que ocupa todo el ancho restante */}
...
export default celebrarteCatalogo;
